...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_loader, dataset = create_dataloader_distill(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 10) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'labels', 'img_size','t_box', 't_obj', 't_cls'))
        if RANK in [-1, 0]:
            pbar = tqdm(pbar, total=nb)  # progress bar
        optimizer.zero_grad()
        for i, (imgs,imgs_teacher, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            # target [图片索引，类别，center_x,center_y,w,h],box尺寸除以了w或h
            ...
            # Backward
            scaler.scale(loss).backward()   # 后向传播
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 5+'%10.4g' * 3) % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, targets.shape[0], imgs.shape[-1],*teacher_mloss)
